...
public class _09_PointsInsideTheHouse {
	...
	// For the triangle (say ABC) and the point P: if the area of ABC = area
	// APB + areaBPC + areaCPA => point is inside;
	boolean inside = pointInRectangle(X, Y, top, bottom,
		leftRectangleLeftBorder, leftRectangleRightBorder)
		|| pointInRectangle(X, Y, top, bottom,
			rightRectangleLeftBorder, rightRectangleRightBorder)
		|| insideTriangle(X, Y);

	System.out.println(inside ? "Inside" : "Outside");
    }

    public static boolean pointInRectangle(double X, double Y, double top,
